...
const std::string DEFAULT_MANAGER_SERVER_IP = "0.0.0.0";
const int DEFAULT_MANAGER_SERVER_PORT = 33333;

class ManagementServer final {
public:
    static const unsigned int NUM_OF_ACTIONS = 2;
...
private:
    ...
    std::map<std::string, std::function<void(ManagementServer*, SOCKET, std::string&)>> m_actionStringMappings;
     ...
     */
    static SOCKET initListeningSocket(const std::string& ip, int port);

    /*
     * @brief Creates an array that the indices are the Actions and the values are their string represention.
     *
     * @return An Array of pointers to strings represeting the Actions.
     */
    static std::map<std::string, std::function<void(ManagementServer*, SOCKET, std::string&)>>
